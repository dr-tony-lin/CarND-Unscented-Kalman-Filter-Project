...
int main() {
  ...
  h.onMessage([&ukf, &target_x, &target_y](uWS::WebSocket<uWS::SERVER> ws,
    ...
    Eigen::VectorXd measurements_L;

    long long timestamp_R;
    Eigen::VectorXd measurements_R;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          iss_L >> px;
          iss_L >> py;
          measurements_L = VectorXd(2);
          measurements_L << px, py;
          iss_L >> timestamp_L;

          MeasurementPackage<SensorType> meas_package_L(timestamp_L, SensorType::LASER,
            measurements_L);
          ...
          iss_R >> theta;
          iss_R >> ro_dot;
          measurements_R << ro, theta, ro_dot;
          iss_R >> timestamp_R;

          MeasurementPackage<SensorType> meas_package_R(timestamp_R, SensorType::RADAR,
            measurements_R);
